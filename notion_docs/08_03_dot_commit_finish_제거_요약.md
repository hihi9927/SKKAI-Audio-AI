...
**100파일 전체 재측정 (run01과 동일 샘플)**

| | run01 (8/1) | v5 (무음 5500) | **v6 (정규화키 + 무음 8000)** |
|---|---|---|---|
| **finish** | **9** | 2 | **0** |
| dot | 114 | 121 | **123** |
| **WER** | **2.79%** | 2.55% | **2.55%** |
| 커밋 | 123 | 123 | 123 |
| 커밋당 토큰 | 12.75 | 12.74 | 12.75 |

**v6 vs run01 파일별: 동일 97 / 개선 3 / 악화 0.** 개선은 0000(6.06%→0.00%), 0003(7.69%→0.00%), 0034(4.65%→2.33%).

**v6 vs v5: 100파일 텍스트 완전 동일.** 정규화 키와 무음 8000ms는 커밋 귀속만 바꿨고 인식 결과는 건드리지 않았다. 커밋 수(123)와 커밋당 토큰(12.75)도 run01과 사실상 같다 — 커밋 단위를 흔들지 않고 finish만 걷어냈다는 직접 지표.

**v6 확정 규칙 분포: context 14 / stable 107 / stall 2.** 규칙 3(정체 확정)이 처음으로 2회 발동했다(무음이 8000ms로 늘어 정체 판정 기회가 생김). `COMMIT-SKIP` 0건 — 커서 스핀도 없다.
...
- **커밋 후 대소문자 재디코딩으로 문장 중복 방출** — 100파일 재측정에서 0016이 `0.00% → 18.18%`로 회귀했다(`"Oh, Papa! Oh, papa! By that..."`). `'Oh, Papa!'`를 커밋한 뒤 모델이 `'Oh, papa!'`로 고쳐 쓰자 `_committed_cursor`의 prefix 매칭 후보 4개가 전부 대소문자를 구분해 실패했고, dot 커밋은 `committed_seg_count`를 올리지 않아 SEG fallback도 막혀 있어서 커서가 `-1`이 됐다 → **텍스트 전체가 미커밋으로 취급** → 매 콜백 재검출(`COMMIT-SKIP` 수십 회) + finish flush가 이미 커밋한 구간까지 재방출. `_asr_key`에 `.lower()`만 넣은 v4는 재커밋만 막고 중복 방출은 그대로였다(오히려 finish 2→3). → prefix 후보에 소문자 변형을 추가(길이 보존될 때만)해서 커서가 정상 전진하도록 수정. v5에서 0016 정상화(`['dot','dot']`, WER 0.00%), `COMMIT-SKIP` 0건.
- **잔여 finish 2건의 성격이 서로 다름 (trailing silence A/B로 분리) → 둘 다 해결** — 같은 2건을 무음 5500ms/8000ms로 돌려 비교했다. 확정 기회는 누적 오디오 2.0초 배수에서만 생기므로 그 수는 `floor((발화+무음)/2.0)`이다.
  - **0055**: 전송량 7.89s로 다음 청크(8.0s)에 **0.11초** 모자랐다. 무음을 8000ms로 늘리자 `dot` 확정 — 하네스 무음 예산 문제이며 실사용 스트림(마이크가 계속 흐름)에서는 발생하지 않는다. → `--trailing-silence-ms` 기본값을 5500 → **8000**으로 변경.
  - **0085**: 8000ms에서도 finish였다. 모델이 롤백 창 안에서 `'house, mother.'` ↔ `'house mother.'`로 **쉼표를 붙였다 뗐다** 진동해 규칙 2(문자열 완전 일치)가 매 청크 불일치하고, 쉼표가 토큰 하나라 규칙 3의 토큰 수 정체도 성립하지 않았다. → 규칙 2의 비교를 **정규화 키**(`_boundary_key`: 대소문자·구두점·공백 제거)로 바꿔 표기 변형을 흡수. 커밋은 최신 텍스트로 하고 비교에만 키를 쓴다. v6에서 `dot` 확정.
  - 두 수정 후 v6에서 **finish 0건**, 100파일 텍스트는 v5와 완전 동일(회귀 없음).
...
- **규칙 3을 켤지 재검토.** v6에서 처음으로 2회 발동했다(무음 8000ms로 정체 판정 기회가 생김). 이제 실효가 있으나, run02에서 오발동으로 WER을 크게 망친 전력이 있어 발동 조건(현재: committed-base 동일 + frontier 한정 + 1청크)이 충분히 좁은지는 더 봐야 한다.
- **`--trailing-silence-ms` 기본값 변경 범위.** LibriSpeech 클라이언트만 8000으로 바꿨다. KsponSpeech(`default=5500`), `run_concurrent_benchmark.py`/`run_multi_speaker_full.py`(`"5500"` 명시 전달)는 각자 기준선이 있어 그대로 뒀다 — 같은 문제가 있는지 미확인.
