...
def main():
    ...
    try:
        ...
        # 4. 음성 인식
        with silent_io():
            result = model.transcribe(audio_data, fp16=False)  # fp16=False로 안정성 확보
        ...
        if lang == "ko":
            kor_text = text
            # 한국어 → 영어 번역 (deep-translator 사용)
            if _gt_available:
                try:
                    eng_text = GoogleTranslator(source='ko', target='en').translate(kor_text)
                except Exception as e:
                    eng_text = f"(영어 번역 실패: {str(e)[:50]})"
            else:
                eng_text = "(영어 번역 필요: pip install deep-translator)"
        else:
            # 원문이 영어면: 영어 전사 확보
            if lang == "en":
                eng_text = text
            ...
            # 영어 → 한국어 번역 (deep-translator 사용)
            if _gt_available:
                try:
                    kor_text = GoogleTranslator(source='en', target='ko').translate(eng_text)
                except Exception as e:
                    kor_text = f"(한국어 번역 실패: {str(e)[:50]})"
            else:
                kor_text = "(한국어 번역 필요: pip install deep-translator)"
