#!/usr/bin/env python3
"""
Qwen3 + DailyTalk integration test runner.

Mirrors test_qwen3_librispeech.py but loads audio/transcripts from
the DailyTalk test.jsonl (split_audio_test entries).

Text field format: "language English<asr_text>TRANSCRIPT [<SEG>]"
  -> extract everything after <asr_text>, strip trailing <SEG>.

speaker_id is derived from the dialog id in the filename (e.g. d1061).
"""
...
def _parse_text(raw_text):
    """Extract clean transcript from 'language English<asr_text>TEXT [<SEG>]'."""
    # strip all <SEG> tokens then extract after <asr_text>
    m = _ASR_TEXT_RE.search(raw_text)
    if m:
        return m.group(1).strip()
    # fallback: strip <SEG> manually
    return re.sub(r'\s*<SEG>', '', raw_text).strip()


def _extract_dialog_id(stem):
    """Return dialog id (e.g. 'd1061') from a filename stem like 'split_0_1_d1061'."""
    m = _DIALOG_ID_RE.search(stem)
    return m.group(1) if m else stem


def find_audio_files(test_jsonl, split_only=True):
    """Load audio entries from test.jsonl.

    Parameters
    ----------
    test_jsonl : str | Path
    split_only : bool
        If True (default), include only entries whose audio path contains
        'split_audio_test' (the pre-segmented evaluation set).
...
async def process_single_file(ws, audio_data, chunk_size_ms=200, send_interval_ms=200, target_lang='ko', trailing_silence_ms=1000):
    ...
    if not finals and partial_last:
        ...
        finals, segment_events, partial_used_as_tail = merge_trailing_partial(
            finals, segment_events, partial_last,
        )
...
async def process_batch(
    ...
    for idx, audio_info in enumerate(targets, start=1):
        ...
        dialog_rows = [r for r in results if r['speaker_id'] == speaker_id]
        dialog_wer = compute_wer_for_rows(dialog_rows)
        ...
        if dialog_wer is not None:
            logger.info('  DIALOG_%s_RUNNING_WER: %.2f%% (%s files)',
                        speaker_id, dialog_wer * 100, len(dialog_rows))
        else:
            logger.info('  DIALOG_%s_RUNNING_WER: N/A (%s files)', speaker_id, len(dialog_rows))
...
def build_summary_payload(results, policy):
    wer_value, dialog_wers = calculate_wer(results, policy=policy, emit_summary=False)
    by_dialog = {}
    for row in results:
        by_dialog.setdefault(row['speaker_id'], []).append(row)
...
def calculate_wer(results, policy=None, emit_summary=True):
    ...
    by_dialog = {}
    for r in results:
        by_dialog.setdefault(r['speaker_id'], []).append(r)

    for dialog_id, rows in by_dialog.items():
        dialog_wers[dialog_id] = compute_wer_for_rows(rows)
    ...
    if emit_summary:
        ...
        logger.info('Number of dialogs: %s', len(by_dialog))
        logger.info('%s\n', '=' * 70)

    return wer, dialog_wers
...
def main():
    ...
    parser.add_argument('--split-only', action=argparse.BooleanOptionalAction, default=True,
                        help='Use only split_audio_test entries (default: True)')
    ...
    if not os.path.isfile(args.test_jsonl):
        logger.error('test.jsonl not found: %s', args.test_jsonl)
        logger.error('Pass the DailyTalk test.jsonl path with --test-jsonl.')
        sys.exit(1)
    ...
    logger.info('Loaded %s files (split_only=%s)', len(files), args.split_only)
