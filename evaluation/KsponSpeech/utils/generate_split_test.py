"""
generate_split_test.py  (KsponSpeech)

test.jsonl의 절반을 문장 중간에서 분절합니다.
  - 어절(eojeol) 수 3개 미만 항목은 eligible pool에서 제외
  ...
  - text 형식: "language Korean<asr_text>{partial_text}"  (<SEG> 없음)

cut 규칙:
  - 어절 단위 경계에서만 cut 허용 (LTokenizer가 어절을 여러 토큰으로 분리 가능)
  - 전체 어절 수 3개 이상인 utterance만 처리
  - cut 후 partial이 반드시 2어절 이상 남아야 함
  - <SEG> 직전 어절(분절 경계)에서는 cut 불가
  - 마지막 어절에서는 cut 불가
"""
...
TEST_JSONL  = _FT_BASE / "data" / "KSponSpeech" / "test.jsonl"
SPLIT_DIR   = _FT_BASE / "data" / "KSponSpeech" / "split_audio_test"
STATE_FILE  = _FT_BASE / "data" / "KSponSpeech" / "split_test_state.json"
...
def count_eojeols(seg_text: str) -> int:
...
def analyze_seg(seg_text: str, aligner_processor, language: str):
    ...
    return seg_parts, seg_info, eojeol_end_indices, before_seg_set, cumsum
...
def main():
    ...
    if args.resume and state_path.exists():
        ...
        eligible = [
            ...
            if count_eojeols(parse_asr_text(e['text'])) >= 3
        ]
    ...
    while idx < len(targets):
        ...
        valid_cuts = sorted(
            j for j, n_ej in eojeol_ends.items()
            if j < n_tokens - 1
