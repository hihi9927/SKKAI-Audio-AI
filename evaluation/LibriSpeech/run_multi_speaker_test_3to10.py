#!/usr/bin/env python3
"""
multi_speaker_test run_03 ~ run_10 벤치마크 러너

run_01(1명), run_02(2명)에 이어 3명~10명 동시 접속 테스트를 순차 실행.
548개 파일을 N개 클라이언트에 균등 분배, 결과를 하나의 JSON으로 병합.

실행 위치: /home/ubuntu/STiTy
  python evaluation/LibriSpeech/run_multi_speaker_test_3to10.py
"""
...
def merge_results(client_metric_jsons: list[Path], merged_path: Path) -> None:
    ...
    for r in all_raw:
        for seg in r.get("segment_metrics", []):
            # 신규 포맷: fsl_sec / decode_sec / trans_sec
            # 구형 포맷: server_fcl_sec / asr_inference_sec / translation_latency_sec
            ...
            if fcl_key in seg:
                fcl_vals.append(seg[fcl_key])
            if asr_key in seg:
                asr_vals.append(seg[asr_key])
            if tl_key in seg:
                tl_vals.append(seg[tl_key])
...
def run_test(n_clients: int, file_ids: list[str]) -> None:
    ...
    sep = "=" * 60
    log(sep)
    log(f" 테스트 {run_name}: 동시 {n_clients}명 / 파일 {len(file_ids)}개")
    log(sep)
...
def main() -> None:
    ...
    log(f"벤치마크 시작: 동시 {START_N}~{END_N}명 → run_{START_N:02d} ~ run_{END_N:02d}")
