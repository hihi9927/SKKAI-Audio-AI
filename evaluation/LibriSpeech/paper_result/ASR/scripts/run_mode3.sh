...
# 사용법: bash run_mode3.sh <split> <scope> <tag> [--clients N] [추가 옵션...]
#   split: test-clean | test-other | dev-clean | dev-other
#   순차: bash run_mode3.sh test-other sample run01
#   순차: bash run_mode3.sh test-other full run01 --limit 50
#   동시: bash run_mode3.sh dev-clean full c16_run01 --clients 16
#
# tag에는 split이 자동으로 박힌다 (dev-clean + c16_run01 → devclean_c16_run01).
# 결과 경로에 split 차원이 없어서, 같은 tag를 다른 split에 쓰면 덮어쓰기가 아니라
# resume으로 합쳐지기 때문이다. serve_mode3.sh에 같은 3인자를 넘길 것.
#
...
PYTHONPATH= "$STITY_PYTHON" "$RUNNER" \
  ...
  --chunk-size-ms 200 \
  --description "mode3 (rule-based/dot-commit, no-rep-dedup) / baseline(1.0.0) / $SPLIT / ASR only, translation off${DESC_SUFFIX}" \
  "${MODE_ARGS[@]}" \
  "${ARGS[@]}"
