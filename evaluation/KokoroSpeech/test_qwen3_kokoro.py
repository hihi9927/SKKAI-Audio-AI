#!/usr/bin/env python3
"""
Qwen3 + Kokoro Speech Dataset integration test runner.

Kokoro Speech Dataset format:
  Audio : KokoroSpeech/{ID}.wav  (22050 Hz stereo/mono → resampled to 16 kHz)
  Labels: metadata.csv           (pipe-delimited: ID|Transcription|Reading)

Each audio clip is an independent short Japanese reading (~3-8 s),
so a fresh WebSocket connection is opened per file to avoid cross-clip context pollution.

Usage examples:
  # All clips (server must be running)
  python test_qwen3_kokoro.py

  # Limit to first 50 clips
  python test_qwen3_kokoro.py --limit 50

  # Auto-start server
  python test_qwen3_kokoro.py --auto-server --model baseline

  # Fast-push mode (no realtime pacing)
  python test_qwen3_kokoro.py --send-interval-ms 0
"""
...
def load_label_map(metadata_csv: Path) -> dict[str, str]:
    """Parse Kokoro metadata.csv (pipe-delimited: ID|Transcription|Reading)."""
    label_map = {}
    with open(metadata_csv, 'r', encoding='utf-8-sig') as f:
        for line in f:
            ...
            text = parts[1].strip()  # Transcription: kanji-kana mixture
            if file_id and text and file_id.lower() != 'id':  # skip header if present
...
def find_audio_files(data_dir: str) -> list[dict]:
    ...
    audio_dir = data_dir / 'KokoroSpeech'
    ...
    label_map = load_label_map(metadata_csv)

    audio_files = []
    for file_id, text in sorted(label_map.items()):
...
def _kata_to_hira(text: str) -> str:
    ...
    Katakana long-vowel mark ー (U+30FC) has no hiragana equivalent; drop it."""
    result = []
    for ch in text:
        cp = ord(ch)
        if 0x30A1 <= cp <= 0x30F6:
            result.append(chr(cp - 0x60))
        elif cp == 0x30FC:  # ー long vowel mark
            pass
        else:
            result.append(ch)
    return ''.join(result)
...
def main():
    parser = argparse.ArgumentParser(description='Qwen3 Kokoro Speech Dataset integration test (Japanese, CER)')
    parser.add_argument('--data-dir', type=str, default=str(DEFAULT_DATA_DIR),
                        help='Root of Kokoro data (contains KokoroSpeech/ and metadata.csv)')
    ...
    if not files:
        logger.error(
            'No Kokoro audio files found in %s\n'
            'Run setup_kokoro.sh first to download the dataset.',
            data_dir,
        )
        sys.exit(1)
