...
**번역은 로컬 MADLAD-400-3B 가 기본이다** — 구글 두 경로가 다 막혔다(v2 키는 403, 무료 gtx 는
이 IP 에서 429). 모델은 첫 번역 호출 때 올라오므로 번역이 없는 실행은 GPU 를 안 문다. 올라오면
fp16 약 7.2GiB 를 쓰니 ASR 서버의 `--gpu-memory-utilization` 과 합쳐 24GiB 를 넘지 않게 잡는다
(0.25 면 ASR 약 6.2GiB + 번역기 7.2GiB).

> 막힌 API 를 그대로 두면 **지연 지표만 조용히 망가진다.** gtx 로 돌렸을 때 실측(test-other
> 5발화): 429 재시도 백오프가 커밋 경로를 막아 FTL 23.27s / 모델런타임 28.26s 였고, 재시도를
> 끄면 4.80s / 0.93s 였다. WER 은 두 경우 모두 3.77% 로 같아서 눈에 띄지 않는다.
