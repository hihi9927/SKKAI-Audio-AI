...
> **AST 지연은 static@8s 4.67초/BLEU 28.74, punct 10.41초/BLEU 34.70**(FLEURS ar→en 283발화),
...
6. **AST 지연 측정** **완료** — `run_fleurs_ar.sh`, FLEURS ar_eg→en 283발화.
   ...
   **단 seg 축은 아랍어에서 불가** — 베이스는 `<SEG>` 미출력, `en-dailytalk-seg` 는 영어 전용.
   static 청크 스윕 2~9초를 찍은 결과 **8초에서 평평해지고(BLEU 29.20) punct 34.70 을
   5.50 못 따라잡는다.** 대신 지연은 4.67초 대 10.41초로 절반이다 —
   **static@8s 가 현재 아랍어의 가장 쓸 만한 운용점이다.**
   seg 파인튜닝은 시급하진 않되 품질 상한(그 5.50)을 올리려면 여전히 필요하다.
