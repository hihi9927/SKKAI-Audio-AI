...
```
consistency = 유사도(조각 번역 합본, full 번역)     # = v1 의 Q
```
...
```
effective(조각) = adequacy(조각) × (1 − contradiction(그 조각 뒤 경계))
...
### 7.1 `effective` 는 같은 `k` 에서만 비교 가능하다

무분절은 경계가 없어 `contradiction` 을 낼 기회 자체가 없다 (정의상 0). 그래서 `effective`
에서 항상 유리하다 — run02 test 에서 무분절 0.7719 > 우리 최고 0.7503.

루프 안에서는 문제가 안 된다. 모든 후보가 **같은 T 격자**를 쓰고 커버리지 요건(§6.4)이 `k`
를 고정하므로 비교가 유효하다. 그러나 **논문 곡선의 y축으로는 쓸 수 없다.** 곡선은
`adequacy` 를 y축으로 하고, `effective`·`contradiction` 은 같은 지연에서의 비교표로 낸다.
...
```
y = adequacy                      x = laal_words (어절)
```

- 우리 프롬프트: T ∈ {2,3,4,6} 4점 + 무분절 1점
- 비교군: **기계 8자분절 / 무분절 / 사람 `current`** 3종. 노브가 없으므로 각 1점
