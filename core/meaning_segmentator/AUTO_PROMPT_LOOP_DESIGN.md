...
로 hypothesis 길이별 바닥 c₀ 를 잰다.

**종전에 여기 적혀 있던 "바닥이 0.01~0.04 라 관측 contra ~0.15 의 대부분은 실제 신호" 는
틀렸다.** 그 숫자(run03: 1-2어절 0.113 → 10어절+ 0.003)는 `deberta-mnli` 측정이고 그
백엔드는 삭제됐다. 현행 `xlmr-anli` 는 바닥이 **길수록 크다** (재측정 `runs/noise_floor_xlmr/`, 문장
611/350/100): German 1-2어절 0.0237 → 15+어절 0.1065, English 0.0645 → 0.1166,
Chinese 0.0743 → 0.1228. 세 타깃 모두 단조 증가고 p50 도 함께 오른다.

**신호가 얼마나 남는지는 짝지어 재야 한다.** 바닥 표(문장 611개)와 관측 경계값(평가 파일
3개)을 그냥 나란히 놓으면 서로 다른 문장을 비교하게 된다. 같은 문장·같은 길이에서
hypothesis 만 (우리 방출) / (오라클 prefix) 로 바꿔 3,000개 경계를 짝지어 재면:

| 길이 | 우리 | 오라클 | 차이 | 차이 p50 | 우리가 더 나쁨 |
|---|---|---|---|---|---|
| 1-2 | 0.0495 | 0.0114 | +0.0381 | +0.0001 | 55% |
| 3-4 | 0.0645 | 0.0312 | +0.0333 | +0.0000 | 28% |
| 5-6 | 0.0580 | 0.0442 | +0.0138 | −0.0001 | 22% |
| 7-9 | 0.0679 | 0.0411 | +0.0269 | −0.0001 | 31% |
| 10-14 | 0.0826 | 0.0711 | +0.0115 | −0.0007 | 30% |
| 15+ | 0.1201 | 0.0956 | +0.0245 | −0.0020 | 31% |

**초과분은 길이에 따라 사라지지 않는다** (+0.011~+0.038, 평평). 바닥의 *비율*은 길이와 함께
오르지만(23%→80%) 목적함수에 들어가는 건 절대값이라, 모든 길이에서 신호가 있다.
중앙값이 ~0 이고 우리가 더 나쁜 경계가 22~31% 뿐이라는 것도 정상이다 — 평균은 꼬리가
만들고, 지표의 일이 그 꼬리를 잡는 것이다.

**그래도 차감은 여전히 안 한다 — 이유가 바뀌었을 뿐이다.** 바닥은 *아무 위치에서나 자른*
prefix 로 재는데 우리 경계는 *고른 자리*다. 그래서 중립적 영점이 아니고, 실제로 관측 `p50`
이 모든 버킷에서 바닥 `p50` 보다 **낮다** (0.0014 vs 0.0016 … 0.0023 vs 0.0085).
`max(0, c − c₀)` 로 빼면 대부분이 0 으로 눌려 지표가 죽는다. 이 바닥은 "이상적인 시스템도
그 방출 길이에서 얼마나 모순으로 보이는가"이지 빼야 할 상수가 아니다. (§13-6)
...
있다** — NLI 잡음 바닥이 hypothesis 길이에 따라 달라지는데 상위 순위 경계는 문장 앞쪽에
몰린다. run03 test 에서 Spearman raw −0.25 가 보정 후 **+0.14 로 뒤집혔고**, run04 dev 에서
gap raw −0.0012 가 보정 후 **+0.0249** 였다.

⚠️ **그 두 실측은 `deberta-mnli`(짧을수록 바닥이 큼) 시절 것이고, 현행 `xlmr-anli` 는 바닥이
길수록 크다** (§5.3 표, `runs/noise_floor_xlmr/`). 편향의 부호가 반대이므로 지금 보정은
상위 순위의 불리를 없애는 게 아니라 **키울 수 있다.** 재측정 전까지 이 값의 부호를 근거로
쓰지 말 것.
...
| `k` 단조 감소는 구조적 보장 | 경계를 빼기만 하므로 조각 수가 반드시 줄어듦 |
| `laal_words` 단조 상승은 **집계 성질** | 런 평균 17/17 단조. 문장 단위는 11.7%가 반대 방향 — LAAL 의 `τ`(소스를 다 들은 시점) 이후를 안 세기 때문. `metrics.laal_words` 에 반례와 계산 과정 |
...
**루프 격자는 최종 격자에서 배수 `2×` 만 뺀 `{1, 1.5, 3} × t_floor`** (`--t-grid`).
조각 번역 호출이 격자 크기에 비례하므로 루프 중에는 부분집합만 쓰고 전체 격자는 최종
test 에서만 돈다. `score` 는 격자 평균이라 **다른 격자로 잰 `score` 끼리는 비교할 수 없다**
— 어떤 격자를 썼는지 `config.json` 에 남는다.

종전에는 `{1.5, 3}` 이라 **`t_floor` 가 빠져 있었다.** 근거가 있던 조합이 아니라, 옛
기본값 `--t-grid 3 6` 이 `min_gap=0` 에서 재현되도록 배수를 역산한 잔재다. 그 사이 격자가
위로 밀려(`min_gap=3` → 루프 `[6,12]` / 최종 `[4,6,8,12]`) 아래 절반이 통째로 빠졌고,
커버리지 요건은 `t_floor` 로 강제하면서 점수는 거기를 안 재는 상태가 됐다 — run03 의
요건 불일치 사고(test 1차 통과율 0.34)와 같은 종류다. 앞·중간·뒤 세 점으로 바꿨다.
...
1. **루프 중에는 격자 부분집합만** (`{1, 1.5, 3} × t_floor`, 4점 중 3점). 전체 격자는
   dev 채택 판정과 최종 평가에서만. 어떤 격자를 썼는지 `config.json` 에 남는다
...
hypothesis 길이에 따라 달라진다. 상위 순위 경계는 문장 앞쪽 = 짧은 hypothesis 에 몰리므로
보정 없이는 위치 편향이 gap 에 그대로 들어간다.

⚠️ **편향의 부호가 백엔드에 달렸고, 현행 백엔드에서 뒤집혔다.** 아래 실측은
`deberta-mnli`(짧을수록 바닥 큼: 1-2어절 0.113, 10어절+ 0.003) 시절 것이다. `xlmr-anli` 는
길수록 크므로(German 0.024 → 0.107) 지금 보정은 방향이 반대일 수 있다 (§5.3). `loop.load_contra_floor` 가 full 번역의 어절 prefix(정의상 무해한 미완성)로
