...
1. **비교군을 같은 자로 재기.** 사람이 쓴 프롬프트를 자동 생성 프롬프트와 직접
   맞붙인다. 사람 프롬프트는 순위 태그를 달지 않으므로 절단이
   불가능하고, 곡선 위의 **점 하나**로 평가된다 (설계 v2 §11.1).
2. **지표 검증.** 사람이 품질 순서를 아는 프롬프트들을 넣었을 때 지표가 그 순서를
   재현하지 못하면, 에이전트가 아니라 지표를 고쳐야 한다는 신호다.

  python -m core.meaning_segmentator.autoseg.scoring.eval_prompt \
      --prompt <프롬프트 파일>.txt \
...
from ..runtime import data, metrics
from ..infra import gateway
from ..infra.gateway import Gateway
from ..loop import _cell, evaluate, target_is_spaced
from ..runtime.pipeline import (add_translate_args, parse_translator_id, GoogleTranslator,
