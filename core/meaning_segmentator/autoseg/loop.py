...
#
# **1200ms 는 사양이지 유도값이 아니다.** 목표 지연을 데이터에서 뽑지 않는 것과 같다.
# 손값들 자체가 1.04~1.39초로 1.34배 흩어져 있어 어떤 값도 넷을 다 재현하지 못한다.
# 1200 은 de·zh·en 을 정확히 재현하고 ja 만 8 -> 7 로 어긋난다. 반대로 1300 은 ja 를
# 맞히는 대신 en 을 3 -> 4 로 올리는데, 그건 **해롭다고 측정된 방향**이다:
# 7언어쌍 min_gap 스윕에서 mg=4 의 effective 가 -0.0361 로 mg=2·3(-0.003)보다 10배
# 나빴다 (README.md 표). 증거가 있는 쪽으로 어긋나지 않게 잡는다.
#
