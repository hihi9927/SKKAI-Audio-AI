...
순위를 보증하지 않기 때문이다. 결과 해석은 `runs/validity/validity_report.md`.
...
백엔드가 오류 심각도 순서를 재현하는지 실측한다.
  ...
  python -m core.meaning_segmentator.autoseg.gates.validity_check --backends nli comet
"""
...
def main() -> int:
    ...
    # 쓰려면 명시적으로 지정할 것: --backends nli xcomet
    ...
    out_dir = Path(args.out) if args.out else (RUNS_DIR / "validity")
    ...
    for name in args.backends:
        ...
        scorer = metrics.make_backend(name, **kw)
