...
label () {  # <이름> <limit인자> <budget>
  ...
  $PY -u core/meaning_segmentator/tools/covost2_label/label_covost2.py \
    ...
    --min-gap 2 --t-floor 3 --batch-size 6 --workers 12 $LIM \
...
echo "===== 소량 검증 (30문장) $(ts) ====="
label smoke_mg2 "--limit 30" 1.0
echo "  exit=$?"; sed -n '/^{/,/^}/p' $N/logs/smoke_mg2.log

# 게이트: 형식이 깨지거나 경계가 안 늘면 본런에 $10 을 태우지 않는다
$PY - <<PY || { echo "!! 게이트 불통과 — 본런 중단"; mark relabel_mg2.failed "smoke gate"; exit 1; }
import json,sys,re
t=open("$N/logs/smoke_mg2.log").read()
m=re.search(r'\{.*\}', t, re.S)
if not m: print("  요약 JSON 없음"); sys.exit(1)
d=json.loads(m.group(0))
ok = d["format_pass"]>=0.90 and d["text_preserved"]>=0.95 and d["mean_boundaries"]>=2.2
print(f"  게이트: format={d['format_pass']} preserved={d['text_preserved']} "
      f"boundaries={d['mean_boundaries']} (mg3 은 1.40) -> {'통과' if ok else '불통과'}")
sys.exit(0 if ok else 1)
PY

echo "===== 본런 (3000문장) $(ts) ====="
label covost2_n3000_run13_mg2 "" 15.0
rc=$?; echo "  exit=$rc $(ts)"; sed -n '/^{/,/^}/p' $N/logs/covost2_n3000_run13_mg2.log
[ $rc -eq 0 ] || { echo "!! 본런 실패"; mark relabel_mg2.failed "label exit=$rc"; exit 1; }

echo "===== prompt_eval 변환 $(ts)  격자=$GRID ====="
$PY core/meaning_segmentator/tools/covost2_label/to_prompt_eval.py \
  --labels $N/labels/covost2_n3000_run13_mg2.jsonl \
  --run-id covost2/n3000 --label auto_run13_mg2 --min-gap 2 \
  --t-grid $GRID --prompt-file $PROMPT 2>&1 | tail -6
mark relabel_mg2.done "ok"
