...
| `meaning_segmentator/autoseg/` | 분절 프롬프트 자동 생성 에이전트 루프. 설계 @meaning_segmentator/AUTO_PROMPT_LOOP_DESIGN.md, 실험 기록·이관 @meaning_segmentator/autoseg/HANDOFF.md |
...
- `autoseg/`는 Letsur AI Gateway(`.env`의 `CLAUDE_API_KEY`)를 쓴다. OpenAI 호환이라 `httpx`만 있으면 되고 로컬 ML 의존성이 없다. 품질 지표 백엔드를 COMET으로 바꾸려면 `HANDOFF.md` §5를 따를 것 — **백엔드 교체 시 타당도 표(§4.1)와 앵커를 반드시 다시 측정**한다.
