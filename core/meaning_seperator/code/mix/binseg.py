...
def fuse_embeddings(
    ...
    if mode == "concat":
        ...
        # 차원이 다를 수 있으므로 각각 PCA로 동일 차원(min_dim)으로 축소 후 평균
...
def detect_binary_segmentation(
    ...
    """
    Binary Segmentation (오프라인 변화점 탐지)

    전체 시퀀스를 재귀적으로 반으로 분할하며 최적 분할점 탐색.
    비용함수: 구간 내 코사인 유사도 합 최대화
             = 구간 간 불일치 최소화

    히트맵 블록 구조를 직접 활용하기 때문에
    짧은 발화에서도 안정적으로 동작.

    max_cp  : 최대 탐지 변화점 수
    min_seg : 최소 구간 길이 (단어 수)
    """
    n = len(embs)
    if n < 2 * min_seg:
    ...
    # 코사인 유사도 누적합 행렬 (구간 비용 계산용)
    sim = normed @ normed.T   # [n, n]

    def seg_cost(s, e):
        """구간 [s, e) 내 평균 코사인 유사도 (클수록 동질적)"""
        if e - s <= 1:
            return 0.0
        return float(sim[s:e, s:e].sum() - (e - s)) / ((e - s) * (e - s - 1) + 1e-9)

    def best_split(s, e):
        """구간 [s, e)에서 최적 분할점과 점수 반환"""
        best_idx, best_score = -1, -1e9
        for m in range(s + min_seg, e - min_seg + 1):
            # 분할 후 두 구간의 동질성 합 - 분할 전 동질성
            score = seg_cost(s, m) + seg_cost(m, e) - seg_cost(s, e)
            if score > best_score:
                best_score = score
                best_idx   = m
        return best_idx, best_score

    # 우선순위 큐로 greedy하게 최대 max_cp개 분할
    import heapq
    segments = [(0, n)]
    changepoints = []
    scores_map = {}
    ...
    # 초기 후보
    heap = []
    idx, score = best_split(0, n)
    if idx >= 0:
        heapq.heappush(heap, (-score, 0, n, idx))

    while heap and len(changepoints) < max_cp:
        neg_score, s, e, m = heapq.heappop(heap)
        score = -neg_score
        if score <= 0:
            break
        changepoints.append(m)
        scores_map[m] = score

        # 새로 생긴 두 구간에서 후보 탐색
        for ns, ne in [(s, m), (m, e)]:
            ni, ns_score = best_split(ns, ne)
            if ni >= 0 and ns_score > 0:
                heapq.heappush(heap, (-ns_score, ns, ne, ni))

    changepoints.sort()

    # 인접 단어 간 코사인 거리 (시각화용 scores)
    adj_scores = [1.0 - float(normed[i] @ normed[i+1])
                  for i in range(n - 1)]

    return changepoints, adj_scores
...
def ensemble_detect(
    ...
    """
    단어 수 기준 방법론 선택:
      모든 길이 → Binary Segmentation (기본)
      short_thresh=0, long_thresh=9999 로 호출하면 항상 BinSeg

    NOUGAT / BOCPD는 g_t 고착 문제로 현재 비활성화.
    """
    # max_cp: 단어 수에 비례해서 최대 변화점 수 설정
    max_cp  = max(1, n_words // 5)
    min_seg = max(2, n_words // 10)

    detected, scores = detect_binary_segmentation(
        embs, max_cp=max_cp, min_seg=min_seg)
    return {
        "method":  "binseg",
        "detected": detected,
        "scores":   scores,
        "extra":    {"max_cp": max_cp, "min_seg": min_seg},
    }
...
def make_figure_one(result: Dict, out_path: str):
    ...
    if scores and method in ("binseg", "cosine_drop"):
        ax2.bar(word_x[:len(scores)], scores,
                width=duration / n_words * 0.8,
                color=m_color, alpha=0.75, label="코사인 거리")
        if method == "binseg":
            mean_s = float(np.mean(scores))
            ax2.axhline(mean_s, color=RED, lw=1.5, linestyle="--",
                        label=f"평균 ({mean_s:.2f})")
            ax2.set_title("Binary Segmentation 분할 점수 (단어 단위)",
                          fontsize=11, fontweight="bold")
        else:
            thresh = extra.get("threshold", COSINE_THRESH)
            ax2.axhline(thresh, color=RED, lw=1.5, linestyle="--",
                        label=f"임계값 ({thresh:.2f})")
            ax2.set_title("Cosine Drop 점수 (단어 단위)",
                          fontsize=11, fontweight="bold")
...
def main():
    ...
    results       = []
    method_counts = {"cosine_drop": 0, "bocpd": 0, "nougat": 0, "binseg": 0}
