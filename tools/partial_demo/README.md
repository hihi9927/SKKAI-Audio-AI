...
- **화면 순서는 언제나 전사 먼저, 번역 나중이다.** 전사(`partial`)는 원문이라 작은 글씨로
  흐르고, 다 찍힌 뒤 `trans`(기본 500ms)만큼 머물렀다가 번역이 큰 글씨로 자리를 잡는다.
  서버가 `partial` 을 `final` 직전에야 주거나(선행 0.1초) 유지 시간에 밀려 대기열로
  들어온 문장도 같은 순서를 밟는다.
...
| `trans` | `500` | 전사를 다 친 뒤 번역으로 넘어가기까지 두는 시간(ms) |
