...
async def process_single_file(ws, audio_data):
    ...
    chunk_size = int(0.1 * SAMPLING_RATE)  # 100 ms
    ...
    for idx, audio_info in enumerate(targets, start=1):
        ...
        try:
            async with websockets.connect(ws_url, ping_interval=None, ping_timeout=None, max_size=10 * 1024 * 1024) as ws:
                await recv_type(ws, 'hello', timeout=8)
                out = await process_single_file(ws, audio)
        except Exception as e:
...
def main():
    ...
    parser.add_argument('--output', type=str, default='qwen3_librispeech_results.json')
    ...
    parser.add_argument('--server-script', type=str, default='Qwen3-ASR/examples/streaming_websocket_server.py')
    ...
    try:
        ...
        results = asyncio.run(process_batch(files, ws_url, args.output, args.policy, args.limit))
