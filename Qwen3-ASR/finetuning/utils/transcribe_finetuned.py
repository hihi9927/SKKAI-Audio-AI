...
def main():
    ...
    p.add_argument("--checkpoint", type=str,
                   default=os.path.join(REPO_ROOT, "Qwen3-ASR/finetuning/qwen3-asr-finetuning-out/checkpoint-600"))
    p.add_argument("--base_model", type=str, default="Qwen/Qwen3-ASR-1.7B")
    p.add_argument("--eval_json", type=str,
                   default=os.path.join(SCRIPT_DIR, "transcribe/eval_clean.json"))
    p.add_argument("--audio_dir", type=str,
                   default=os.path.join(SCRIPT_DIR, "data/eval_clean"))
    p.add_argument("--output", type=str,
                   default=os.path.join(SCRIPT_DIR, "finetuned_results/eval_clean.json"))
    ...
    print(f"[3/3] {len(data)}개 샘플 전사 시작\n" + "=" * 60)
    ...
    output = {
        **original,
        "stats": {**original.get("stats", {}), "checkpoint": args.checkpoint, "base_model": args.base_model},
        "data": records,
    }
