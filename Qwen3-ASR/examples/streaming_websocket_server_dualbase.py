...
class StreamingConfig:
    ...
    # 한국어 전용 base (en과 base model이 다를 때 별도 vLLM 인스턴스로 로드).
    # None이거나 model_path와 같으면 단일 인스턴스로 동작(기존 동작과 동일).
    model_path_ko: Optional[str] = None
...
class Qwen3ASRStreamingHandler:
    ...
    def __init__(
        ...
        asr_model_ko: Optional[Qwen3ASRModel] = None,
        ...
        self.asr = asr_model
        # ko 전용 base 인스턴스 (없으면 asr와 동일 → 단일 인스턴스 동작)
        self.asr_ko = asr_model_ko if asr_model_ko is not None else asr_model
        self.config = config
    ...
    def _canonical_lang(self, state) -> str:
        """state/클라이언트 설정에서 canonical 언어명(e.g. "Korean")을 해석."""
        lang = state.force_language or state.language
    ...
    def _get_lora_request(self, state):
        ...
        lang = self._canonical_lang(state)
        if lang == "English":
    ...
    def _active_asr(self, state):
        """언어에 따라 실행할 ASR 인스턴스 반환.

        한국어는 ko 전용 base(asr_ko), 그 외(영어/기타/auto)는 표준 multilingual
        base(asr). asr_ko가 없거나 asr와 동일하면 항상 단일 인스턴스를 사용한다.
        _get_lora_request와 동일한 언어 판정을 공유하므로 (인스턴스, LoRA) 짝이 항상 일치한다.
        """
        if self.asr_ko is None or self.asr_ko is self.asr:
            return self.asr
        if self._canonical_lang(state) == "Korean":
            return self.asr_ko
        return self.asr
    ...
    async def _asr_streaming_transcribe(self, chunk: np.ndarray, slot_key: Optional[str] = None):
        ...
        try:
            await self._active_asr(slot["state"]).streaming_transcribe(chunk, slot["state"], lora_request=lora_request, on_seg=_on_seg)
        finally:
            self._in_generate_loop = False
    ...
    async def _asr_finish_streaming(self, slot_key: Optional[str] = None):
        ...
        async with self.asr_lock:
            ...
            await self._active_asr(state).finish_streaming_transcribe(state, lora_request=lora_request)
    ...
    async def _retry_vad_short_utterance(self, slot_key: str) -> None:
        ...
        async with self.asr_lock:
            ...
            await self._active_asr(fresh_state).finish_streaming_transcribe(fresh_state, lora_request=lora_request)
...
class Qwen3ASRStreamingServer:
    ...
    def __init__(self, config: StreamingConfig):
        ...
        self.asr = None
        self.asr_ko = None  # ko 전용 base 인스턴스 (dual-base 모드에서만 생성)
    ...
    def init_model(self):
        ...
        # ── 인스턴스 구성 결정 ──────────────────────────────────────────────
        # model_path_ko가 지정되고 en base와 다르면 ko 전용 base를 별도 인스턴스로 로드.
        # (en LoRA와 ko LoRA의 base model이 다른 경우 한 vLLM 엔진에 함께 올릴 수 없다.)
        ko_base = (self.config.model_path_ko or "").strip()
        two_instance = bool(ko_base) and os.path.abspath(ko_base) != os.path.abspath(self.config.model_path)

        # 두 인스턴스를 한 GPU에 올릴 때는 메모리를 분할(과도한 값으로 인한 OOM 방지)
        gpu_mem = self.config.gpu_memory_utilization
        if two_instance:
            gpu_mem = min(gpu_mem, 0.45)
            logger.info(f"Dual-base mode 활성화: en base + ko base, gpu_memory_utilization={gpu_mem} (each)")
        ...
        # ── en / 표준 multilingual 인스턴스 ─────────────────────────────────
        # dual-base 모드에서는 이 인스턴스가 영어/기타/auto를 담당하고 en LoRA만 사용한다.
        en_lora = bool(adapter_en_path) if two_instance else use_lora
        logger.info(f"Loading model (en/standard base): {self.config.model_path}")
        self.asr = Qwen3ASRModel.LLM(
            ...
            max_model_len=8192,
            enable_lora=en_lora,
            max_lora_rank=self.config.max_lora_rank if en_lora else 16,
            enforce_eager=self.config.enforce_eager,
        )
        self.asr.sampling_params = _build_sampling_params()

        # ── ko 전용 base 인스턴스 (dual-base 모드에서만) ─────────────────────
        if two_instance:
            ko_lora = bool(adapter_ko_path)
            logger.info(f"Loading model (ko base): {ko_base}")
            self.asr_ko = Qwen3ASRModel.LLM(
                model=ko_base,
                gpu_memory_utilization=gpu_mem,
                max_new_tokens=self.config.max_new_tokens,
                max_model_len=8192,
                enable_lora=ko_lora,
                max_lora_rank=self.config.max_lora_rank if ko_lora else 16,
                enforce_eager=self.config.enforce_eager,
            )
            self.asr_ko.sampling_params = _build_sampling_params()

        # ── LoRA 어댑터 등록 ───────────────────────────────────────────────
        # LoRARequest는 특정 엔진에 묶이지 않는 디스크립터다. en LoRA는 asr에서만,
        # ko LoRA는 asr_ko에서만 사용되도록 _active_asr/_get_lora_request가 짝을 보장한다.
        if use_lora:
            try:
    ...
    async def start(self):
        ...
        if self.asr_ko is not None and self.asr_ko is not self.asr:
            logger.info("Warming up ko base model...")
            await warmup_streaming(self.asr_ko)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Model path or name (en/표준 multilingual base)",
    )
    parser.add_argument(
        "--model-ko", type=str, default=None,
        help="한국어 전용 base model 경로 (en과 base가 다를 때 별도 vLLM 인스턴스로 로드). "
             "미지정 또는 --model과 같으면 단일 인스턴스로 동작.",
    )
    parser.add_argument(
