...
배치 네 가지를 `1`~`4` 키(또는 우상단 버튼)로 바꾼다. 선택은 localStorage 에 남는다.
...
| `4` | 목표별 칸 | 언어마다 가로 띠. 태그가 없고 원문·번역도 구분하지 않는다 — 칸이 곧 언어라 그 칸에는 그 언어 문장만 쭉 흐른다. 말한 언어의 칸에는 원문이 그대로 들어간다 |

칸을 쓰는 배치(`2`·`4`)는 **칸 이름을 왼쪽에 크게 고정한다.** 칸마다 같은 자리·같은 크기라
어느 칸인지 한눈에 잡힌다. 글 뭉치는 그 이름과 같은 높이에 온다 — 아래에 붙이면 줄이 적을 때
이름과 어긋난다.
