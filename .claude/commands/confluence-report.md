...
- **소제목에 번호를 붙이고, 그 아래는 짧게.** 큰 갈래는 `# 1. …`, 그 안은 `## 1.1 …`.
  방식·단계·항목처럼 셀 수 있는 것은 번호 제목 하나에 내용 몇 줄이다. 한 소제목 아래
  문단이 셋을 넘으면 갈래를 더 나눈다. 남은 것·코드 위치도 번호로 가리킬 수 있게 된다.
- **한 문장에 한 가지.** 20자 안팎의 짧은 문장. 쉼표로 두 가지를 잇지 않는다.
- **소제목 아래 첫 문장이 결론.** 근거와 수치는 그 뒤에.
- **팀이 이미 아는 용어는 풀이하지 않는다.** VAD, 디코딩, ASR, partial/final, `<SEG>`,
  슬롯, whisper, COMET 같은 것에 괄호 설명을 달면 오히려 읽기 거슬린다. 풀이는 **이 업무에서
  새로 생긴 것**에만 — 새 방법, 새 지표, 새 이름. 그것도 처음 한 번, 문장으로.
- **코드 안의 이름(함수·플래그·파일)은 그대로 쓴다.** 이름을 임의로 바꿔 부르면 검색이 안
  된다. 무슨 일을 하는지는 그 이름이 처음 나오는 문장이 말해 주면 된다 — 매번 괄호를 달지
  않는다.
...
이렇게 쓴다:
> ## 2.3 발화가 끝나면 다시 판정해 경로를 교정한다
> 발화 전체를 다시 판정한다. 첫 판정과 다르면 그 발화를 맞는 서버로 다시 보내고, 먼저
> 나가려던 자막은 버린다.
...
(`| a | b |`)도 쓸 수 있다 — 실측값 비교는 표가 읽기 좋다. md 의 `#` 은 항목 소제목과
같은 크기(h4), `##` 은 그 아래(h5)로 그려진다. 큰 갈래는 `#`, 그 안은 `##` 까지만.
