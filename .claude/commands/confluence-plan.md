...
문서는 `문서 정리 > <n>차 업무 분담 > 계획 문서` 폴더에 들어가고, 제목은 `[n차][대범주][소범주] 계획 문서`,
계획/보고 구분은 **라벨**로 한다.
...
- `<n>차 업무 분담` 폴더가 없으면 `문서 정리` 아래에 자동으로 만들고,
  그 아래 `계획 문서` / `보고 문서` 폴더도 없으면 만든다
- 같은 제목이 이미 있으면 **덮어쓰지 않고 소범주 태그 옆에 번호를 붙인다** —
  `[1차][시연 대비][아랍어 지원][2] 계획 문서`. 비어 있는 다음 번호를 자동으로 찾는다
- 번호를 직접 정하려면 JSON 최상단에 `"seq": 3` 을 넣는다
- 번호를 붙이지 않고 예전처럼 멈추게 하려면 `--no-autonumber` 를 준다
- `--dry-run` 이 번호가 붙었다고 알려주면, 기존 문서를 고칠 생각이었는지 사용자에게 확인해줘
